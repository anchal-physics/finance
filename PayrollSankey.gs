 ...
 * PayrollSankey.gs
 * ----------------------------------------------------------------------
 * Server endpoints for the PayrollSankey landing tab: bootstrap payload,
 * snapshot/polling, cell writes, row add/delete/move, per-user settings +
 * subpanel locks, the effective-range override, and addNewLevel.
 *
 * Same Apps Script project as Webapp.gs (the top-level combiner), so it
 * shares Webapp.gs's helpers (setCellSmart_, moveRowOnSheet_, safeGetUserEmail_,
 * getReorderAck/setReorderAck) and SankeyRenderer.gs's constants
 * (PRESET_RANGE, TARGET_SHEET_NAME, COLUMNS_PER_GROUP) via global scope.
 * ----------------------------------------------------------------------
 */
